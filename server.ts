...
app.get(['/api/auth/instagram/callback', '/api/auth/instagram/callback/'], async (req, res) => {
  ...
  try {
    ...
    if (tokenData.error || !tokenData.access_token) {
      ...
      console.warn('Meta Instagram token error:', errorMsg);
      return res.send(`
        <!DOCTYPE html>
        <html>
          <head><title>Instagram Authorization Failed</title></head>
    ...
    if (profileDisplayName === 'Instagram User') {
      try {
        ...
        console.warn('Could not fetch Meta /me fallback:', uErr);
      }
    }
...
app.get(['/api/auth/facebook/callback', '/api/auth/facebook/callback/'], async (req, res) => {
  ...
  try {
    ...
    if (tokenData.error || !tokenData.access_token) {
      const errorMsg = tokenData.error?.message || tokenData.error_description || 'Failed to obtain access token from Meta';
      console.warn('Meta Facebook token error:', errorMsg);
      return res.send(`
        <!DOCTYPE html>
        <html>
          <head><title>Facebook Authorization Failed</title></head>
    ...
    try {
      ...
      const userData = await userRes.json();
      console.log('Meta Facebook /me user profile:', userData);
      ...
      console.warn('Could not fetch Facebook /me user info:', uErr);
    }
